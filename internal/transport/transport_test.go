...
func (s) TestClientSendsAGoAwayFrame(t *testing.T) {
	connWriterHung = make(chan struct{})
	// Create a server.
	lis, err := net.Listen("tcp", "localhost:0")
	if err != nil {
		t.Fatalf("Error while listening: %v", err)
...
var connWriterHung chan struct{}
...
func (hc *hangingConn) Write(b []byte) (n int, err error) {
	...
	if n == 42 { // GOAWAY frame
		close(connWriterHung)
	}
	return n, err
}
...
func createClientServerConn(t *testing.T, connOptions ConnectOptions) (context.Context, chan error) {
	// Create a server.
	lis, err := net.Listen("tcp", "localhost:0")
	if err != nil {
		t.Fatalf("Error while listening: %v", err)
	...
	fmt.Println("Connect Options: ", connOptions)
	ct, err := NewClientTransport(ctx, context.Background(), resolver.Address{Addr: lis.Addr().String()}, connOptions, func(GoAwayReason) {})
	if err != nil {
		t.Fatalf("Error while creating client transport: %v", err)
	}
	_, err = ct.NewStream(ctx, &CallHdr{})
